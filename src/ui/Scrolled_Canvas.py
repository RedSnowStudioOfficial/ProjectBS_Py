...
class ScrolledCanvas:
    def __init__(self, master, weight=640, height=480):
        Label(master, text="ScrolledCanvas:", wraplength="4i", justify=LEFT).pack(side=TOP)
    ...
    def scrollLeave(self, event):
        ...
        if 'text' in self.canvas.gettags(CURRENT):
            id = id-1
        self.canvas.itemconfigure(id, fill='background')
    ...
    def scrollButton(self, event):
        ...
        if ids:
            ...
            if not 'text' in self.canvas.gettags(CURRENT):
                id = id+1
            print("you ckicked on %s" % \
                self.canvas.itemcget(id, 'text') )            

if __name__ == '__main__':
    root = Tk()
    root.option_add('*Font', 'Vardana 12')
